...
function Get-ReportData {
    ...
    $sql = "SELECT COUNT(*) AS num FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA = '$SCHEMA' AND LOWER(TABLE_NAME) = '$Table'"
    write-verbose $sql
...
function Save-ReportData {
    ...
    PROCESS {
        foreach ($object in $InputObject) {
            if (-not $SetUp) {
                $table = Test-Database -ConnectionString $ConnectionString -Object $object -schema $Schema # -Debug -verbose
                $SetUp = $True
            }
...
function Test-Database {
    ...
    # Test to see if table exists
    $sql = "SELECT COUNT(*) AS num FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA = '$SCHEMA' AND LOWER(TABLE_NAME) = '$($Table.tolower())'"
    write-verbose $sql
    $cmd = New-Object -TypeName System.Data.SqlClient.SqlCommand
    $cmd.CommandText = $sql
    $cmd.Connection = $conn
    ...
    if ($num_rows -gt 0) {
        ...
        foreach ($property in $Properties) {
            ...
            if ($object.($property) -is [int] -or
                ...
                $object.($property) -is [uint32]) {
                $sql += "[$property] INTEGER"
            } elseif ($object.($property) -is [int64] -or
                $object.($property) -is [uint64]) {
                $sql += "[$property] BIGINT"
            } elseif ($object.($property) -is [datetime]) {
                $sql += "[$property] DATETIME2"
            } elseif ($object.($property) -is [bool]) {
                $sql += "[$property] bit"
            } else {
